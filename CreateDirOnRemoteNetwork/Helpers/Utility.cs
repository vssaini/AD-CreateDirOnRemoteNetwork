...
namespace CreateDirOnRemoteNetwork.Helpers
{
    /// <summary>
    ...
    class Utility
    {
        ...
        /// <summary>
        /// Create directory in Shared folder over UNC path.
        /// </summary>
        /// <param name="networkName">The network name as \\192.168.1.202\Data\</param>
        /// <param name="directoryFullName">The network directory as \\192.168.1.202\Data\VikramDirectory</param>       
        public static void CreateDirectoryInShare(string networkName, string directoryFullName)
        {
            if (PathIsNetworkPath(directoryFullName))
            {
                var nw = GetNetworkCredential();
                using (new NetworkConnection(networkName, nw))
                {
                    // Create directory
                    var dirInfo = new DirectoryInfo(directoryFullName);
                    
                    var dirExists = dirInfo.Exists;
                    if (!dirExists)
                        dirInfo.Create();
                }
            }
